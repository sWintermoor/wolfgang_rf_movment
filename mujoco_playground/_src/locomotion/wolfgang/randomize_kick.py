...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand_dynamics(rng):
    ...
    rng, key = jax.random.split(rng)
    frictionloss = model.dof_frictionloss[6:24] * jax.random.uniform(
        key, shape=(18,), minval=0.9, maxval=1.1
    )
    dof_frictionloss = model.dof_frictionloss.at[6:24].set(frictionloss)

    # Scale armature: *U(1.0, 1.05).
    rng, key = jax.random.split(rng)
    armature = model.dof_armature[6:24] * jax.random.uniform(
        key, shape=(18,), minval=1.0, maxval=1.05
    )
    dof_armature = model.dof_armature.at[6:24].set(armature)
