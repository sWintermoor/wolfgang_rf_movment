...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand_dynamics(rng):
    ...
    rng, key = jax.random.split(rng)
    frictionloss = model.dof_frictionloss[13:25] * jax.random.uniform(  #TODO: Richtig?
        key, shape=(12,), minval=0.9, maxval=1.1
    )
    dof_frictionloss = model.dof_frictionloss.at[13:25].set(frictionloss) #TODO: Richtig?

    # Scale armature: *U(1.0, 1.05).
    rng, key = jax.random.split(rng)
    armature = model.dof_armature[13:25] * jax.random.uniform( #TODO: Richtig?
        key, shape=(12,), minval=1.0, maxval=1.05
    )
    dof_armature = model.dof_armature.at[13:25].set(armature) #TODO: Richtig?
